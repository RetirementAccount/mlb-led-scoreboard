...
**Two real bugs found and fixed getting pause/skip working, both confirmed via live testing:**
...
2. **The actual root cause of "rapid skip presses reset the screen instead of advancing"** (found by adding temporary diagnostic logging to both services and correlating exact timestamps from a live test, rather than continuing to guess) — `__draw_plugin_screen`'s loop only checked `cond()` (and therefore only checked for a pending skip) once per plugin's own `wait_time()`. Fast plugins (news/standings, ~0.05s cadence) responded to every individual press fine; every `espn_sports` plugin (`wait_time() = 4.0s`) only looked for a skip once every 4 seconds, so any extra rapid presses landing inside that window were silently dropped — multiple presses collapsed into a single consumed skip. Fixed by decoupling the two: `__draw_plugin_screen` now polls `cond()` at a fixed `CONTROL_POLL_INTERVAL` (0.05s) independent of `wait_time`, while actual rendering still only happens on the plugin's own cadence. Confirmed fixed by rapid-pressing skip on an NFL/NHL/NBA screen after deploying.
