...
3. **MLB games: rapid skip got stuck re-showing the same game** — a genuinely different bug, in the data layer rather than the render loop's polling rate. `self.data.games.next()` (called by `__render_games`) doesn't itself advance anything; it reads from a `DoubleBuffer` (`data/utils/double_buffer.py`) that a *separate thread* (the main data-fetching loop in `main.py`) fills in, via a two-phase mirror-then-fetch handoff that only completes roughly once per main-loop refresh cycle (~0.5-1s). Skipping faster than that just re-reads the same still-stale "next" game over and over — confirmed via logs showing "Entering game 2/15" repeated many times with no progress. This is an inherent producer/consumer latency in the existing double-buffering architecture (built for thread-safety between the data and render threads, predating this session), not something to eliminate outright.
   ...
   - **Not a bug, but looks like one**: `priority: 2` (a live Cubs game, per the `teams: ["Cubs"]` rule in `config.json`) narrows `schedule.num_games()` down to just that one game, *and* every plugin screen is only registered at `with_priority: [0, 1]` (see the earlier rotation-toggles section) — so while a Cubs game is live, skip has nothing else available to advance to at all, by original project design. Confirmed by checking `statsapi.schedule()` directly during a session where skip appeared to "only refresh" — a Cubs game genuinely was live at the time (`824634`, Brewers @ Cubs, in progress). Worth remembering next time this comes up: check whether a Cubs game is live before assuming skip is broken.
